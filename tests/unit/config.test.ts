import { getServers, saveServer, removeServer, findServer, SERVERS_FILE, CONFIG_DIR } from '../../src/utils/config';
...
describe('config', () => {
  ...
  describe('getServers', () => {
    ...
    it('should return parsed servers from file', () => {
      const servers = [
        { id: '123', name: 'test', provider: 'hetzner', ip: '1.2.3.4', region: 'nbg1', size: 'cax11', createdAt: '2026-01-01T00:00:00Z' },
      ];
  ...
  describe('saveServer', () => {
    it('should create config dir and write server', () => {
      mockedFs.existsSync.mockReturnValueOnce(false) // ensureConfigDir
        .mockReturnValueOnce(false); // getServers: file doesn't exist
      mockedFs.readFileSync.mockReturnValue('[]');

      const record = { id: '1', name: 'srv', provider: 'hetzner', ip: '1.2.3.4', region: 'nbg1', size: 'cax11', createdAt: '2026-01-01T00:00:00Z' };
      saveServer(record);
    ...
    it('should append to existing servers', () => {
      const existing = [{ id: '1', name: 'old', provider: 'hetzner', ip: '1.1.1.1', region: 'nbg1', size: 'cax11', createdAt: '2026-01-01T00:00:00Z' }];
      mockedFs.existsSync.mockReturnValue(true);
      mockedFs.readFileSync.mockReturnValue(JSON.stringify(existing));

      const record = { id: '2', name: 'new', provider: 'digitalocean', ip: '2.2.2.2', region: 'nyc1', size: 's-2vcpu-2gb', createdAt: '2026-02-01T00:00:00Z' };
      saveServer(record);
  ...
  describe('removeServer', () => {
    it('should remove server by id and return true', () => {
      const servers = [
        { id: '1', name: 'a', provider: 'hetzner', ip: '1.1.1.1', region: 'nbg1', size: 'cax11', createdAt: '' },
        { id: '2', name: 'b', provider: 'hetzner', ip: '2.2.2.2', region: 'fsn1', size: 'cx23', createdAt: '' },
      ];
      mockedFs.existsSync.mockReturnValue(true);
      mockedFs.readFileSync.mockReturnValue(JSON.stringify(servers));
  ...
  describe('findServer', () => {
    const servers = [
      { id: '1', name: 'alpha', provider: 'hetzner', ip: '10.0.0.1', region: 'nbg1', size: 'cax11', createdAt: '' },
      { id: '2', name: 'beta', provider: 'digitalocean', ip: '10.0.0.2', region: 'nyc1', size: 's-2vcpu-2gb', createdAt: '' },
    ];
