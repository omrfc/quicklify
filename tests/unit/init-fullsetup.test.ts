...
const mockedFirewallSetup = firewallModule.firewallSetup as jest.MockedFunction<typeof firewallModule.firewallSetup>;
const mockedSecureSetup = secureModule.secureSetup as jest.MockedFunction<typeof secureModule.secureSetup>;
  ...
  mockedAxios.get.mockImplementation((url: string) => {
    ...
    if (url.includes('server_types')) {
      return Promise.resolve({
        data: {
          server_types: [
            { name: 'cax11', cores: 2, memory: 4, disk: 40, prices: [{ location: 'nbg1', price_monthly: { gross: '3.85' } }] },
          ],
        },
      });
    }
...
describe('init --full-setup', () => {
  ...
  it('should call firewallSetup and secureSetup when --full-setup and Coolify ready', async () => {
    ...
    expect(mockedSecureSetup).toHaveBeenCalledWith('10.0.0.1', 'test-server', undefined, false, true);
  });
