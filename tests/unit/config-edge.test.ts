...
describe('config edge cases', () => {
  ...
  describe('saveServer edge cases', () => {
    it('should not create dir if it already exists', () => {
      mockedFs.existsSync.mockReturnValueOnce(true) // ensureConfigDir - dir exists
        .mockReturnValueOnce(false); // getServers - file doesn't exist

      const record = { id: '1', name: 'test', provider: 'hetzner', ip: '1.1.1.1', region: 'nbg1', size: 'cax11', createdAt: '' };
      saveServer(record);
  ...
  describe('removeServer edge cases', () => {
    it('should handle removing from single-element array', () => {
      const servers = [{ id: '1', name: 'a', provider: 'hetzner', ip: '1.1.1.1', region: 'nbg1', size: 'cax11', createdAt: '' }];
      mockedFs.existsSync.mockReturnValue(true);
    ...
    it('should not modify file when server not found', () => {
      mockedFs.existsSync.mockReturnValue(true);
      mockedFs.readFileSync.mockReturnValue(JSON.stringify([{ id: '1', name: 'a', provider: 'hetzner', ip: '1.1.1.1', region: '', size: '', createdAt: '' }]));
  ...
  describe('findServer edge cases', () => {
    ...
    it('should find server by IP when name also matches another', () => {
      const servers = [
        ...
        { id: '2', name: '10.0.0.1', provider: 'digitalocean', ip: '10.0.0.2', region: 'nyc1', size: 's-2vcpu-2gb', createdAt: '' },
      ];
      mockedFs.existsSync.mockReturnValue(true);
      mockedFs.readFileSync.mockReturnValue(JSON.stringify(servers));
