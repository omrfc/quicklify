...
must_haves:
  ...
  key_links:
    ...
    - from: "src/commands/restore.ts"
      to: "src/utils/modeGuard.ts"
      via: "isBareServer for mode-aware routing"
      pattern: "isBareServer"
---
...
<task type="auto" tdd="true">
  <name>Task 2: Route backup/restore commands by server mode with SAFE_MODE enforcement</name>
  ...
  <behavior>
    ...
    - Test: restoreCommand on bare server with SAFE_MODE=true exits with error and does NOT call restoreBareBackup
    - Test: restoreCommand on coolify server with SAFE_MODE=true exits with error (existing behavior preserved)
  </behavior>
  <action>
    ...
    2. **src/commands/restore.ts** -- Route by mode with explicit SAFE_MODE enforcement:
       - Import `isBareServer` from `../utils/modeGuard.js`
       - Import `restoreBareBackup` from `../core/backup.js`
       - SAFE_MODE check must happen BEFORE the mode branch, at command level:
         ```typescript
         if (process.env.SAFE_MODE === "true") {
           logger.error("Restore is blocked by SAFE_MODE. Set SAFE_MODE=false to allow restore operations.");
           return;
         }
         ```
         This ensures bare restore is blocked identically to Coolify restore when SAFE_MODE is enabled.
       - After SAFE_MODE check, resolve server, then check `isBareServer(server)`:
    ...
    4. Add explicit SAFE_MODE test for bare restore: mock `process.env.SAFE_MODE = "true"`, mock resolveServer to return a bare server, call restoreCommand, assert logger.error is called with SAFE_MODE message and restoreBareBackup is NOT called.
  </action>
  <verify>
    <automated>npx jest tests/unit/backup.test.ts tests/unit/restore.test.ts --no-coverage 2>&1 | tail -10</automated>
  </verify>
  <done>Backup/restore commands route to bare or coolify functions based on server mode, SAFE_MODE blocks bare restore with explicit error, existing behavior unchanged</done>
</task>
...
- SAFE_MODE=true blocks bare restore with clear error message
</verification>
...
- SAFE_MODE=true blocks bare restore with explicit error (tested, not just assumed)
