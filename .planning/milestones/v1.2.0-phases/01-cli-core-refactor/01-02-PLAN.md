...
<task type="auto">
  ...
  <action>
NOTE: This task owns ALL changes to `src/commands/domain.ts` — both constant imports AND pure function removal. Plan 01-01 intentionally does NOT touch this file to avoid parallel execution conflicts.
...
4. For constants (`COOLIFY_SOURCE_DIR`, `COOLIFY_DB_CONTAINER`, `COOLIFY_DB_USER`, `COOLIFY_DB_NAME`): Import them from `../constants.js` (Plan 01-01 extracts these to `src/constants.ts`). If any constant is still needed locally in `commands/domain.ts` for string matching (e.g., `COOLIFY_DB_CONTAINER` used in stdout checks), import from `../constants.js`.

5. Check `grep -r "COOLIFY_DB_CONTAINER" src/commands/domain.ts` — if it's used for string matching in the command (checking stdout includes container name), import it from `../constants.js` where Plan 01-01 centralizes it.
