...
must_haves:
  truths:
    ...
    - "Health command on bare server prints error and exits: command requires Coolify"
    ...
    - "Destroy, secure, firewall, domain, snapshot commands work unchanged on bare servers"
...
<objective>
Make status and list commands mode-aware, and add Coolify-only guards to health, update, maintain, and logs commands. Add regression tests confirming destroy, secure, firewall, domain, and snapshot work unchanged on bare servers.

Purpose: Bare servers must show cloud-only status (no Coolify health check), appear correctly in list output with a Mode column, and Coolify-specific commands must refuse to run on bare servers with a clear error. Per user decision, health is Coolify-specific and must error on bare servers (not show "n/a"). Infra commands (destroy, secure, firewall, domain, snapshot) already work on any server and need regression tests to verify the mode field does not break them.
Output: Mode-aware status/list display, Coolify-only guards on health/update/maintain/logs, regression tests for infra commands.
</objective>
...
<task type="auto" tdd="true">
  <name>Task 1: Mode-aware status and list commands + core status</name>
  <files>src/core/status.ts, src/commands/status.ts, src/commands/list.ts, tests/unit/status.test.ts, tests/unit/list.test.ts, tests/unit/core-status.test.ts</files>
  <behavior>
  ...
  <action>
    ...
    4. Update tests for each command to verify mode-aware behavior.
  </action>
  <verify>
    <automated>npx jest tests/unit/status.test.ts tests/unit/list.test.ts tests/unit/core-status.test.ts --no-coverage 2>&1 | tail -10</automated>
  </verify>
  <done>Status shows cloud-only for bare, list has Mode column, all existing Coolify behavior unchanged</done>
</task>

<task type="auto" tdd="true">
  <name>Task 2: Add Coolify-only guards to health, update, maintain, logs commands + regression tests for infra commands</name>
  <files>src/commands/health.ts, src/commands/update.ts, src/commands/maintain.ts, src/commands/logs.ts, tests/unit/health.test.ts, tests/unit/update.test.ts, tests/unit/maintain.test.ts, tests/unit/logs.test.ts, tests/unit/destroy.test.ts, tests/unit/secure.test.ts, tests/unit/firewall.test.ts, tests/unit/domain.test.ts, tests/unit/snapshot.test.ts</files>
  <behavior>
    - Test: healthCommand on bare server exits with error "health command is not available for bare servers"
    - Test: healthCommand on coolify server proceeds normally (existing behavior)
    ...
    - Test: destroyCommand with bare-mode ServerRecord succeeds (BARE-03 regression)
    - Test: secureCommand with bare-mode ServerRecord succeeds (BARE-04 regression)
    - Test: firewallCommand with bare-mode ServerRecord succeeds (BARE-05 regression)
    - Test: domainCommand with bare-mode ServerRecord succeeds (BARE-06 regression)
    - Test: snapshotCommand with bare-mode ServerRecord succeeds (snapshot regression)
  </behavior>
  <action>
    1. **src/commands/health.ts** -- Add Coolify-only guard (per user decision: health is Coolify-specific, must error on bare):
       - Import `requireCoolifyMode` from `../utils/modeGuard.js`
       - After `resolveServer()`, check mode:
         ```typescript
         const modeError = requireCoolifyMode(server, "health");
         if (modeError) {
           logger.error(modeError);
           return;
         }
         ```
       - For `--all` mode: filter out bare servers from the list, log a warning for each skipped bare server
       - Existing Coolify health check logic unchanged

    2. **src/commands/update.ts** -- Add Coolify-only guard:
       - Import `requireCoolifyMode` from `../utils/modeGuard.js`
       - After `resolveServer()`, check mode:
         ```typescript
    ...
    3. **src/commands/maintain.ts** -- Add Coolify-only guard:
       - Import `requireCoolifyMode` from `../utils/modeGuard.js`
    ...
    4. **src/commands/logs.ts** -- Service-aware guard:
       - Import `isBareServer` from `../utils/modeGuard.js`
    ...
    5. **Regression tests for infra commands (BARE-03/04/05/06 + snapshot):**
       These commands do NOT need code changes -- they operate at cloud API or SSH level, which is mode-independent. However, regression tests MUST verify they accept bare-mode ServerRecords without error.

       For each of these existing test files, add a test case using a bare-mode server fixture (`{ ...baseServer, mode: "bare" }`):

       - **tests/unit/destroy.test.ts**: Add test "destroyCommand with bare-mode server succeeds" -- mock resolveServer to return a bare server, verify destroyCommand proceeds to the provider.destroyServer call without mode-related errors
       - **tests/unit/secure.test.ts**: Add test "secureCommand with bare-mode server succeeds" -- mock resolveServer to return a bare server, verify secureCommand proceeds to SSH hardening without mode-related errors
       - **tests/unit/firewall.test.ts**: Add test "firewallCommand with bare-mode server succeeds" -- mock resolveServer to return a bare server, verify firewallCommand proceeds to UFW setup without mode-related errors
       - **tests/unit/domain.test.ts**: Add test "domainCommand with bare-mode server succeeds" -- mock resolveServer to return a bare server, verify domainCommand proceeds to domain/SSL setup without mode-related errors
       - **tests/unit/snapshot.test.ts**: Add test "snapshotCommand with bare-mode server succeeds" -- mock resolveServer to return a bare server, verify snapshotCommand proceeds to provider snapshot call without mode-related errors

    6. Update health/update/maintain/logs tests to verify guards.
  </action>
  <verify>
    <automated>npx jest tests/unit/health.test.ts tests/unit/update.test.ts tests/unit/maintain.test.ts tests/unit/logs.test.ts tests/unit/destroy.test.ts tests/unit/secure.test.ts tests/unit/firewall.test.ts tests/unit/domain.test.ts tests/unit/snapshot.test.ts --no-coverage 2>&1 | tail -15</automated>
  </verify>
  <done>Health/update/maintain blocked on bare servers with clear error. Logs service=coolify blocked on bare, system/docker logs work. Destroy, secure, firewall, domain, snapshot all pass regression tests with bare-mode servers. All coolify behavior unchanged.</done>
</task>
...
- health/update/maintain on bare server prints error and exits
- logs --service coolify on bare server prints error
- destroy/secure/firewall/domain/snapshot work with bare-mode servers (regression tests pass)
...
- Health command on bare server: exits with error "requires Coolify" (per user decision)
...
- destroy, secure, firewall, domain, snapshot: regression tests pass with bare-mode servers (BARE-03/04/05/06)
