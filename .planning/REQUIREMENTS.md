...
- [x] **REF-03**: `src/commands/init.ts`'deki ~320 satırlık `deployServer()` fonksiyonu `src/core/deploy.ts`'ye extract edilir, init.ts thin wizard wrapper kalır

### Security

- [x] **SEC-01**: `scpDownload()`/`scpUpload()` stdin `"ignore"` yapılır ve `-o BatchMode=yes` eklenir (MCP JSON-RPC stream corruption önleme)
- [x] **SEC-02**: `scpDownload()`/`scpUpload()`'a timeout eklenir (default 300s, ağ kesildiğinde CLI hang önleme)
